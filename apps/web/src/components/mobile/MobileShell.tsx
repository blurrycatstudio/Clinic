 ...
 * (with a springy overshoot) then slowly swaying left-right in place while active, so
    ...
    return (
      <img
        ...
        className={cn(
          ...
          isActive ? "size-8 animate-tab-sway" : "size-6",
        )}
      />
    )
  }
  return (
    <tab.icon
      className={cn(
        ...
        isActive ? "size-6 animate-tab-sway" : "size-5",
      )}
      strokeWidth={2}
    />
  )
